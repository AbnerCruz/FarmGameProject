...
//Maybe this class change completely
                ...
                if(coordsX >= 0 && coordsX < principal_grid.base_grid.GetLength(0) && coordsY >= 0 && coordsY < principal_grid.base_grid.GetLength(1)){
                    ...
                    switch(current_cell){
                        ...
                        case 1: //Crystal Farm Cell
                            manager.player.player_energy += principal_grid.farm_grid[coordsX,coordsY].farm_current_storage;
                            principal_grid.farm_grid[coordsX,coordsY].farm_current_storage = 0;
                            manager.audioManager.item_drop_audio.Play();
                        break;
                        case 2: //Extractor Cell
                            manager.player.player_rocks += Random.Range(1,10);
                            manager.player.player_crystals += principal_grid.farm_grid[coordsX,coordsY].farm_current_storage;
                            principal_grid.farm_grid[coordsX,coordsY].farm_current_storage = 0;
                            manager.audioManager.item_drop_audio.Play();
                        break;
                    }
