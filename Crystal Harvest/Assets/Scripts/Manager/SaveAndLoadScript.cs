...
public class SaveAndLoadScript
{
    public void Save(GameManager manager, GridClass target_grid, string filePath){
        Directory.CreateDirectory(Path.GetDirectoryName(filePath)); //Directoy to save file
        ...
        player_data.player_nickname = player.player_nickname;
    ...
    public void Load(GameManager manager, GridClass target_grid, string filePath){
        ...
        if(File.Exists(filePath+"_player.data")){
            ...
            player.player_nickname = player_data.player_nickname;
    ...
    GridClass GridLoad(GameManager manager, GridClass target_grid, string filePath){
        ...
        if(File.Exists(filePath+"_world.data")){
            ...
            //World Loading
            foreach(Transform child in manager.manager_tile_grid.transform){
                manager.ManagerDestroy(child.gameObject);
            }
            if(manager.show_debugs){
                foreach(Transform child in manager.manager_text_grid.transform){
                manager.ManagerDestroy(child.gameObject);
                }
            }
            foreach(Transform child in manager.manager_build_tile_grid.transform){
                manager.ManagerDestroy(child.gameObject);
            }
            foreach(Transform child in manager.manager_accumulated_farms_grid.transform){
                manager.ManagerDestroy(child.gameObject);
            }
            ...
            grid.highlight_grid = new GameObject[manager.grid_width, manager.grid_height];
            ...
            grid.AccumulatedDropsTextDebug(manager);
            grid.GridTiles(manager);
            for(int x = 0; x < grid.ore_life_grid.GetLength(0); x++){
                for(int y = 0; y < grid.ore_life_grid.GetLength(1); y++){
                    if(grid.ore_life_grid[x,y] <= 0){
                        if(grid.player_buyed_tiles[x,y] == 0){
                            grid.tile_grid[x,y].GetComponent<SpriteRenderer>().sprite = manager.manager_tile_grid.GetComponent<TileGridScript>().breaked_tile_sprite;
                        }
                        else{
                            grid.tile_grid[x,y].GetComponent<SpriteRenderer>().sprite = manager.manager_tile_grid.GetComponent<TileGridScript>().flat_rock_tile;
                        }
                    }
                }
            }
            manager.GridBuildUpdater(grid);
            for(int x = 0; x < grid.ore_life_grid.GetLength(0); x++){
                for(int y = 0; y < grid.ore_life_grid.GetLength(1); y++){
                    if(grid.farm_grid[x,y] != null){
                        grid.farm_grid[x,y].farm_current_storage = grid.farm_storage_grid[x,y];
                    }
                }
            }
        }
